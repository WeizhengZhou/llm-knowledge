"""
build_pdf_cn_full.py — Generates a PDF from all 13 Chinese-language chapters.

Embeds Noto Sans CJK SC font so the PDF renders correctly on all devices.

Usage:
    python3 build_pdf_cn_full.py

Output:
    output/stem-guidebook-all-cn.pdf
"""
...
CHAPTERS = [
    ...
    os.path.join(SCRIPT_DIR, "cn_05-middle-school-years.md"),
    os.path.join(SCRIPT_DIR, "cn_06-high-school-years.md"),
    os.path.join(SCRIPT_DIR, "cn_07-summer-programs.md"),
    os.path.join(SCRIPT_DIR, "cn_08-training-and-resources.md"),
    os.path.join(SCRIPT_DIR, "cn_09-acceleration.md"),
    os.path.join(SCRIPT_DIR, "cn_10-college-admissions.md"),
    os.path.join(SCRIPT_DIR, "cn_11-burnout.md"),
    os.path.join(SCRIPT_DIR, "cn_12-the-real-cost.md"),
    os.path.join(SCRIPT_DIR, "cn_13-girls-in-competitive-stem.md"),
]

OUTPUT_PDF = os.path.join(OUTPUT_DIR, "stem-guidebook-all-cn.pdf")

TITLE         = "培养STEM竞赛人才：家长实战指南"
SUBTITLE      = "从幼儿园到高中 — 竞赛、项目与真正有效的方法"
CHAPTER_RANGE = "完整版（第一至十三章）"
DATE          = "2026年4月"
...
# ---------------------------------------------------------------------------
# CSS — slightly larger base font (12pt vs 11pt)
# ---------------------------------------------------------------------------
...
li {{ margin-bottom: 0.35em; line-height: 1.85; }}
...
def main():
    ...
    with tempfile.NamedTemporaryFile(
        mode="w", suffix=".html", encoding="utf-8", delete=False, prefix="stem_cn_full_"
    ) as tmp:
        tmp.write(html)
        tmp_path = tmp.name
